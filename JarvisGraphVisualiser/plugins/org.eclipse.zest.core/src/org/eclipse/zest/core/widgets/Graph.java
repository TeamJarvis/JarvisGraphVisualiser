...
public class Graph extends FigureCanvas implements IContainer {
	...
	public HashSet getSubgraphFigures() {
		return subgraphFigures;
	}

	public void setSubgraphFigures(HashSet subgraphFigures) {
		this.subgraphFigures = subgraphFigures;
	}
	...
	public void setSelectionListeners(List selectionListeners) {
		this.selectionListeners = selectionListeners;
	}

	public HashMap getFigure2ItemMap() {
		return figure2ItemMap;
	}

	public void setFigure2ItemMap(HashMap figure2ItemMap) {
		this.figure2ItemMap = figure2ItemMap;
	}
	...
	private class DragSupport implements MouseMotionListener,
		...
		Point previousDragLocation = null;
		...
		public void mouseDragged(org.eclipse.draw2d.MouseEvent me) {
			...
			if (selectedItems.isEmpty()) {
				...
				if (subgraphFigures.contains(figureUnderMouse)) {
					...
					org.eclipse.swt.graphics.Point graphSize = getGraphSize();
					if (graphSize != null) {
						org.eclipse.swt.graphics.Point parentSize = getParent()
								.getSize();
						if ((graphSize.x > parentSize.x - 5)
								&& (graphSize.y > parentSize.y - 5)) {
							Point viewLocation = getViewport()
									.getViewLocation();
							Point draggLocation = new Point(me.x, me.y);
							int x = viewLocation.x + draggLocation.x
									- previousDragLocation.x;
							int y = viewLocation.y + draggLocation.y
									- previousDragLocation.y;
							Point newViewLocation = new Point(x, y);
							getViewport().setViewLocation(newViewLocation);
							previousDragLocation = draggLocation;
							return;
						}
					}
				}
		...
		public void mousePressed(org.eclipse.draw2d.MouseEvent me) {
			...
			previousDragLocation = dragStartLocation.getCopy();
		...
		public org.eclipse.swt.graphics.Point getGraphSize() {
			if (nodes.size() == 0) {
				return null;
			}
			GraphNode node0 = (GraphNode) nodes.get(0);
			int minX = node0.getLocation().x;
			int minY = node0.getLocation().y;
			int maxX = node0.getLocation().x + node0.getSize().width;
			int maxY = node0.getLocation().y + node0.getSize().height;
			for (int i = 1; i < nodes.size(); i++) {
				GraphNode node = (GraphNode) nodes.get(i);
				int minXNode = node.getLocation().x;
				int minYNode = node.getLocation().y;
				int maxXNode = node.getLocation().x + node.getSize().width;
				int maxYNode = node.getLocation().y + node.getSize().height;
				if (minXNode < minX) {
					minX = minXNode;
				}
				if (minYNode < minY) {
			...
			return new org.eclipse.swt.graphics.Point(
					(int) ((maxX - minX) * getRootLayer().getScale()),
					(int) ((maxY - minY) * getRootLayer().getScale()));
		}

		public org.eclipse.swt.graphics.Point getGraphLocation() {
			if (nodes.size() == 0) {
				return null;
			}
			GraphNode node0 = (GraphNode) nodes.get(0);
			...
			return new org.eclipse.swt.graphics.Point(minX, minY);
		}

		private void moveViewpoint() {
			org.eclipse.swt.graphics.Point graphLocation = getGraphLocation();
			Point glp = new Point(graphLocation.x, graphLocation.y);
			getRootLayer().translateToRelative(glp);
			getViewport().setViewLocation(glp);
		}

		private void printPoint(String point, Point p) {
	...
	public Graph getCopy(Composite parent, int style) {
		Graph copy = new Graph(parent, style);
		copy.setNodes(nodes);
		copy.setPreferredSize(preferredSize);
		copy.setConnectionStyle(connectionStyle);
		copy.setNodeStyle(nodeStyle);
		copy.setConnections(connections);
		copy.setSubgraphFigures(subgraphFigures);
		copy.setSelectedItems(selectedItems);
		copy.setSelectionListeners(selectionListeners);
		copy.setFigure2ItemMap(figure2ItemMap);
		return copy;
	}
}
